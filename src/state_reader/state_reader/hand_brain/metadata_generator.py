...
def set_metadata_value(event):
    ...
    if "session_start_time" in full_key:
        ...
        entryedit.set_date(datetime.strptime(current_value, '%Y/%m/%d %H:%M').date())
        ...
        def on_date_time_select():
            ...
            selected_datetime = datetime(selected_date.year, selected_date.month, selected_date.day, selected_hour, selected_minute)
            formatted_datetime = selected_datetime.strftime('%Y/%m/%d %H:%M')
...
def save_metadata_to_csv():
    global final_folder_name
    if messagebox.askokcancel("Save Data", " Do you want to save data"):
        file_path = filedialog.asksaveasfilename(defaultextension=".csv", filetypes=[("CSV files", "*.csv"), ("All files", "*.*")])
        if not file_path:
            return
        try:
            def convert_session_to_str(num_str):
                num = int(num_str.split('_')[-1])
                return str(num).zfill(3)
            
            date_str = metadata['NWBFile']['session_start_time']
            date_time = datetime.strptime(date_str, '%Y/%m/%d %H:%M')
            formatted_date = date_time.strftime('%Y%m%d')

            session_str = metadata['NWBFile']['session_number']
            session = convert_session_to_str(session_str)

            task_name = metadata['NWBFile']['experiment_name']
            folder_name = f'{formatted_date}_{task_name}_{session}'
            folder_path = os.path.dirname(file_path)
            final_folder_name = os.path.join(folder_path,folder_name)
            if os.path.exists(final_folder_name):
                shutil.rmtree(final_folder_name)
                os.mkdir(final_folder_name)
            else:
                os.mkdir(final_folder_name)
            tmp_file_name = os.path.basename(file_path)
            
            with open(os.path.join(final_folder_name,tmp_file_name), mode='w', newline='') as file:
