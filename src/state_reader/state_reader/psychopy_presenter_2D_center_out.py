...
def main(args=None):
    ...
    for trial in BMIExp:
        ...
        while TrialTimeCounter.getTime()<10:
            ...
            psychopy_presenter.get_logger().info('Publishing: Intention state: {}'.format(str(intention_state.state)))
            psychopy_presenter.intention_state_publisher_.publish(intention_state)
            
            #%% actual state
            state_msg.state = list(object_instance['Cursor'].pos)
            psychopy_presenter.get_logger().info('Publishing: State: {}'.format(str(state_msg.state)))
