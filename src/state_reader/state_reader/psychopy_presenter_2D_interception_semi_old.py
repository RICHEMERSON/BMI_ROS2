...
try:
    ...
    AssistCoefficient1 = shared_memory.ShareableList([0],name='AssistCoefficient1')
    ...
    for trial_index, trial in enumerate(BMIExp):
        ...
        while TrialTimeCounter.getTime()<decoding_time:
            ...
            if feedforward_flag and np.linalg.norm(object_instance['Cursor'].pos)<10 and once_ff:
                ...
                # 0.15, tau = 0.25
                Xvel = np.cos(ff_diff_degree)*InitialSpeed*AssistGenerator(BellCurveCounter,tau=0.075)*AssistCoefficient1[0]+\
                        np.cos(speed_degree)*FeedforwardSpeed*AssistGenerator(BellCurveCounter,tau=0.075)*GenerationSpeed[0]+\
                        np.cos(DiffDegree)*AssistCoefficient2[0]
                
                Yvel = np.sin(ff_diff_degree)*InitialSpeed*AssistGenerator(BellCurveCounter,tau=0.075)*AssistCoefficient1[0]+\
                        np.sin(speed_degree)*FeedforwardSpeed*AssistGenerator(BellCurveCounter,tau=0.075)*GenerationSpeed[0]+\
        ...
        # sBMIExp.saveAsPickle(psychopy_filename)
