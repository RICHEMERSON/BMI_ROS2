...
class BlackrockActiveIrPublisher(Node):
    ...
    def __init__(self):
        ...
        #%% declare parameters
        self.declare_parameter('parameters', 0.5)
        ...
        self.declare_parameter('spiketrain_window', 0.5)
        ...
        self._spiketrain_window = self.get_parameter('spiketrain_window').get_parameter_value().double_value
        ...
        self.get_logger().info('Parameters: spiketrain_window: {}'.format(str(self._spiketrain_window)))
        ...
        super().__init__('/system_{}/blackrock_active_ir_publisher'.format(str(system)))
        ...
        self.srv = self.create_service(
                ActiveObservation, 
                '/system_{}/active_observation'.format(str(system)), 
                self.neural_vector_callback
                )
        ...
        self._neural_buffer_deque = [deque() for _ in range(chn_num)] # a deque buffer for saving spiketrain
	...
	#%% build the ros timer
        self.timer = self.create_timer(timer_period, self.build_spiketrain_callback)
    ...
    def build_spiketrain_callback(self):
        ...
        if len(event)!=0:
            for i in event: 
                self._neural_buffer_deque[i[0]-1] = self._neural_buffer_deque[i[0]-1] + i[1]['timestamps'][0]
