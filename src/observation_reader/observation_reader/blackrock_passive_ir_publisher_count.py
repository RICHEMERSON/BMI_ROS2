...
class BlackrockPassiveIrPublisher(Node):
    ...
    def __init__(self):
        ...
        super().__init__('system_BlackrockPassiveIrPublisher')
        ...
        # self._neural_buffer = np.memmap('/share/nb.bin', dtype='float64', mode='w+', shape=(parameters['chn_num'],))
        self._neural_buffer = np.zeros((parameters['chn_num'], ))
    ...
    def neural_array_callback(self):
        ...
        if len(self._neural_buffer_deque)==self._neural_buffer_deque.maxlen:
            ...
            self._neural_array = self._neural_buffer
