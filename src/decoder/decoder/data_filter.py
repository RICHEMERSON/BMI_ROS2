...
self.publisher_{} = self.create_publisher(
                PassiveObservation, output_{}_groups[{}].format(str(system),str(group)), 1
                )
                """
...
def _declare_subscriber(index, data_type):
    ...
    return subscribe_template.format(
           group_name, data_type, data_type, index, group_name, group_name
           )
...
def _declare_subscriber_callback(index, data_type):
    ...
    return subscribe_function_template.format(
           group_name, group_name, data_type, index, data_type, index, group_name, data_type
           )

def _synchro_concatenation_subscriber(self, shared_buffer, group_name, data_type):
    
    concatenate_buffer = []
    po_msg = interface_dict[data_type]
    
    for i in shared_buffer:
        if len(i)==0:
            concatenate_buffer.clear()
            break
        else:
            concatenate_buffer = concatenate_buffer + i[-1]
