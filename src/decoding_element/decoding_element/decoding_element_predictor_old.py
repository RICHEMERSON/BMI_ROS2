...
def model_inference(observation_q, state_q, decoding_element_q):
    ...
    while True:
        ...
        if decoding_element is not None:
            ...
            decoding_state = list(decoding_element.predict(decoding_input).squeeze())
...
class DecodingElementPredictor(Node):
    ...
    def neural_data_listener_callback(self, msg):
        ...
        if not readout is None:
            self.get_logger().info('Publishing: decoding state: {}, input: {}'.format(str(self._decoding_state), readout[1], list(pickle.dumps(readout[2]))))
