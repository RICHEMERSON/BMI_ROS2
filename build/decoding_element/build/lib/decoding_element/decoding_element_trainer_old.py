...
def trainer(decoding_element, sample_buffer, de_buffer):
      ...
      while True: 
         ...
         if not sample_buffer.empty():
             ...
             select_trial_index = state_data[state_data[:,1::].sum(1)==0]
             select_state_data = state_data[state_data[:,1::].sum(1)!=0]
             
             select_neural_data = np.array([select_neural_data[select_trial_index[:,0]==i] for i in np.unique(select_state_data[:,0])]).squeeze()
             print(select_neural_data.shape)
             
             decoding_element.fit(np.array(select_neural_data), np.array(select_state_data)[:,1::])
...
class DecodingElementTrainer(Node):
    ...
    def __init__(self):
        ...
        trainer_process = Process(target=trainer, args=(parameters, self._decoding_element, self._sample_buffer, self._de_buffer))
