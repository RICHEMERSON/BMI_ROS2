    ...
    for trial_index, trial in enumerate(BMIExp):
        ...
        while TrialTimeCounter.getTime()<decoding_time:
            ...
            if np.linalg.norm(object_instance['Cursor'].pos)>1.5 and not feedforward_flag and ring_flag[0]!=0:
                ...
                decoding_time = TrialTimeCounter.getTime()+1.5
            ...
            psychopy_presenter.get_logger().info('Publishing: coefficient: decoder coefficient: {}, assist coefficient: {}, PosFlag: {}, GenerationSpeed: {}, AssistCoefficient1: {}, AssistCoefficient2: {}, moving_target_flag: {}, training flag: {}'.format(
                        str(decoder_coefficient[0]), str(assist_coefficient[0]), str(PosFlag[0]), str(GenerationSpeed[0]), str(AssistCoefficient1[0]), str(AssistCoefficient2[0]), str(moving_target_flag[0]), str(tf))
                        )
