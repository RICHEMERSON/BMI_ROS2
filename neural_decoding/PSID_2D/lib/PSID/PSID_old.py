...
def blkhankskip(Y, i, j=None, s=0, time_first=True):
    ...
    if isinstance(Y, (list, tuple)):
        if j == None:
            j = [None for yi in range(len(Y))]
        HList = [blkhankskip(Y[yInd], i, j[yInd], s, time_first=time_first) for yInd in range(len(Y))]
        H = np.concatenate(HList, axis=1)
...
def PSID(Y, Z=None, nx=4, n1=3, i=4, WS=dict(), return_WS=False, fit_Cz_via_KF=False, time_first=True):
    """
