...
def raise_observation_reader_node(recording_system, node_mode, log_file, **kwargs):
    ...
    return p
time_dir = '/home/soma/data/bmi_data/leb/'+str(int(time.time()*10))+'_observation'+'/'
if not os.path.exists(time_dir):
    os.mkdir(time_dir)

log_file = time_dir+'system{}_observation.log'.format(str(parameters['system']))
observation_p = raise_observation_reader_node('blackrock', 'passive', log_file, **parameters)
